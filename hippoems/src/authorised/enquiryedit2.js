...
export default function Enquiryedit(){
  ...
  const [data,setData]=useState({user_id:"",refernce:"",owner:'',company:'',person:'',gst:'',email:'',number:'',state:'',city:'',subject:"",source:'',sector:'',stage:'',customertype:'',status:"",duedate:"",specialnotes:""});
  ...
  const [ownes,setOwners]=useState([]);
  const enquirystatus=['pending','accept','complete','reject']

  useEffect(()=>{

    axios.get(`${process.env.REACT_APP_HIPPOEMS}/owners`).then((res)=>{
      if(res.status===200){
       setOwners(res.data)
      }
      else{
  ...
  const ownerhandler=(e)=>{
    ...
    const owner=value.username;
    setData({...data,user_id: ownerid,owner: owner});
  ...
  const update=()=>{
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        axios.post(`${process.env.REACT_APP_HIPPOEMS}/adminupdate/${eid}`,data).then((res)=>{
          if(res.status === 200){
        ...
        <Grid container spacing={3}>
          ...
          <FormControl style={{width:'96%'}} margin="normal" variant="outlined">
              ...
              >
                ...
                {ownes.map((item, index) => (
                  <MenuItem key={index} value={{ username: item.username, id: item.id }}>
                    {item.username}
                  </MenuItem>
                ))}
              </Select>
